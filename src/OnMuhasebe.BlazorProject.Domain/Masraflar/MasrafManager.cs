...
public class MasrafManager : DomainService
{
    ...
    public MasrafManager(IMasrafRepository masrafRepository, IBirimRepository birimRepository, IOzelKodRepository ozelKodRepository)
    {
    ...
    {
        await _masrafRepository.RelationalEntityAnyAsync(x => x.FaturaHareketler.Any(y => y.MasrafId == id));
    }
}