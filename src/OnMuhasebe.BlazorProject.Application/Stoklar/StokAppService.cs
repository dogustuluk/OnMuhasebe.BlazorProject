...
public class StokAppService : BlazorProjectAppService, IStokAppService
{
    ...
    {
        var entity = await _stokRepository.GetAsync(id, x => x.Id == id, x => x.Birim, x => x.OzelKod1, x => x.OzelKod2);
    ...
    public virtual async Task<PagedResultDto<ListStokDto>> GetListAsync(StokListParameterDto input)
    {
        var entities = await _stokRepository.GetPagedListAsync(input.SkipCount, input.MaxResultCount, x => x.Durum == input.Durum, x => x.Kod);
    ...
    public virtual async Task<SelectStokDto> UpdateAsync(Guid id, UpdateStokDto input)
    {
        ...
        await _stokManager.CheckUpdateAsync(id, input.Kod, entity, input.BirimId, input.OzelKod1Id, input.OzelKod2Id);
