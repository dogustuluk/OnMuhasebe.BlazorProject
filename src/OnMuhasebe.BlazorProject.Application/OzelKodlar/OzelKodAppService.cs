...
public class OzelKodAppService : BlazorProjectAppService, IOzelKodAppService
{
    ...
    {
        var entities = await _ozelKodRepository.GetPagedListAsync(input.SkipCount, input.MaxResultCount, x => x.KodTuru == input.KodTuru && x.KartTuru == input.KartTuru && x.Durum == input.Durum, x => x.Kod);
        ...
        return new PagedResultDto<ListOzelKodDto>(totalCount, ObjectMapper.Map<List<OzelKod>, List<ListOzelKodDto>>(entities));
    }
