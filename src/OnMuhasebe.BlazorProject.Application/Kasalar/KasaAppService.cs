...
public class KasaAppService : BlazorProjectAppService, IKasaAppService
{
    ...
    {
        var entity = await _kasaRepository.GetAsync(id, x => x.Id == id, x => x.OzelKod1, x => x.OzelKod2); //Sube ile ilgili bir property kullanmayacağımız için include etmedik. İsteğe bağlı olarak include edilebilir.
        return ObjectMapper.Map<Kasa, SelectKasaDto>(entity);
    }

    public virtual async Task<string> GetCodeAsync(KasaCodeParameterDto input)
    {
        return await _kasaRepository.GetCodeAsync(x => x.Kod, x => x.SubeId == input.SubeId && x.Durum == input.Durum);
    }

    public virtual async Task<PagedResultDto<ListKasaDto>> GetListAsync(KasaListParameterDto input)
    {
        var entities = await _kasaRepository.GetPagedListAsync(input.SkipCount, input.MaxResultCount, x => x.SubeId == input.SubeId && x.Durum == input.Durum, x => x.Kod, x => x.OzelKod1, x => x.OzelKod2, x => x.MakbuzHareketler);//Eğer MakbuzHareketler'in de içerisinde herhangi bir navigation property'e ihtiyacımız varsa burda include etmeyip, EfCoreKasaRepository sınıfında include etmeliyiz.
        var totalCount = await _kasaRepository.CountAsync(x => x.SubeId == input.SubeId && x.Durum == input.Durum);
